...
La lista stazioni NON è più hardcoded: viene scaricata da STATIONS_URL
(un file JSON raw pubblicato su GitHub) ad ogni avvio e ad ogni pressione
di R. Se il download fallisce (offline), viene usata l'ultima copia
scaricata con successo, salvata in CACHE_FILE. Se non esiste nemmeno
quella, si usa DEFAULT_STATIONS come ultima rete di sicurezza.

Per aggiornare le stazioni su TUTTI i player: modifica il file JSON nel
repo GitHub, fai commit/push, e premi R (o riavvia) su ogni player.

Formato del file JSON remoto — una lista di oggetti:
[
  {"name": "M2O", "url": "https://..."},
  {"name": "Radio Deejay", "url": "https://..."}
]
"""
...
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  CONFIGURAZIONE LISTA STAZIONI REMOTA                                        ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
# Sostituisci con l'URL raw del file JSON nel tuo repo GitHub, es.:
# https://raw.githubusercontent.com/<utente>/<repo>/main/stations.json
STATIONS_URL = "https://marzorati.co/download/radio_stations.json"

CACHE_FILE  = Path.home() / ".cache" / "radio_player" / "stations_cache.json"
FETCH_TIMEOUT = 5  # secondi

# Rete di sicurezza se non c'è né connessione né cache locale
DEFAULT_STATIONS = [
...
def _validate_stations(data):
    """Ritorna la lista pulita se il JSON ha la forma attesa, altrimenti None."""
    if not isinstance(data, list) or not data:
        return None
    cleaned = []
    for item in data:
        if isinstance(item, dict) and item.get("name") and item.get("url"):
            cleaned.append({"name": str(item["name"]), "url": str(item["url"])})
    return cleaned or None


def fetch_remote_stations(url=STATIONS_URL, timeout=FETCH_TIMEOUT):
    """Scarica la lista stazioni dall'URL. Ritorna la lista o None se fallisce."""
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "radio_player/2.4"})
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            raw = resp.read().decode("utf-8")
        return _validate_stations(json.loads(raw))
    except (urllib.error.URLError, TimeoutError, ValueError, OSError):
        return None


def load_cached_stations():
    """Legge l'ultima lista salvata in cache. Ritorna la lista o None."""
    try:
...
def load_stations():
    """
    Prova, in ordine: download da GitHub -> cache locale -> DEFAULT_STATIONS.
    Ritorna (lista_stazioni, messaggio_stato).
    """
    remote = fetch_remote_stations()
    if remote:
        save_cached_stations(remote)
        return remote, f"✅ Lista aggiornata da GitHub ({len(remote)} stazioni)"

    cached = load_cached_stations()
    if cached:
        return cached, f"⚠️  Offline: uso cache locale ({len(cached)} stazioni)"

    return list(DEFAULT_STATIONS), "⚠️  Nessuna connessione e nessuna cache: lista di default"
...
def main(stdscr):
    ...
    stations, load_msg = load_stations()
    player        = RadioPlayer()
    ...
    while True:
        ...
        if key in (ord("q"), ord("Q")):
            ...
            player.status_msg = "🔄 Aggiornamento lista da GitHub…"
            draw_screen(stdscr, stations, selected, scroll_offset, player, viz, frame)
            stations, load_msg = load_stations()
